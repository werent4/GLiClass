...
from gliclass.training import RLTrainer, RLTrainerConfig, Trainer, TrainingArguments
...
def main(args):
    ...
    if args.set_value_model:
        value_model = AutoModelForSequenceClassification.from_pretrained(
            model.config.encoder_model_name, num_labels=1
        )
    ...
    if args.reference_model is not None:
        ...
        reference_pipe = ZeroShotClassificationPipeline(
            refrence_model,
            reference_tokenizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", type=str, default="knowledgator/gliclass-modern-base-v2.0-init"
    )
    parser.add_argument(
        "--encoder_model_name", type=str, default="microsoft/deberta-v3-small"
    )
    parser.add_argument(
        "--label_model_name", type=str, default="BAAI/bge-small-en-v1.5"
    )
