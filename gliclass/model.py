...
from transformers import AutoConfig, AutoModel, PreTrainedModel, Wav2Vec2Model
...
class GLiClassBaseModel(nn.Module):
    def __init__(self, config: GLiClassModelConfig, device="cpu", **kwargs):
        ...
        if config.pooling_strategy not in POOLING2OBJECT:
            raise NotImplementedError(
                f"{config.pooling_strategy} is not implemented pooling type."
            )
        else:
        ...
        if config.use_lstm:
            self.lstm = LstmSeq2SeqEncoder(
                config.hidden_size, config.hidden_size // 2, bidirectional=True
            )

        if config.squeeze_layers:
            self.layer_wise_attention = LayerwiseAttention(
                config.encoder_config.num_hidden_layers,
    ...
    def _extract_class_features(self, token_embeds, input_ids, attention_mask):
        ...
        aranged_class_idx = torch.arange(
            max_embed_dim, dtype=attention_mask.dtype, device=token_embeds.device
        ).expand(batch_size, -1)

        batch_indices, target_class_idx = torch.where(
            aranged_class_idx < num_class_tokens
        )
        ...
        classes_embedding[batch_indices, target_class_idx] = token_embeds[
        ...
        if self.config.extract_text_features:
            ...
            aranged_token_idx = torch.arange(
                input_ids.shape[-1], device=token_embeds.device
            ).unsqueeze(0)

            batch_idx, text_token_idx = torch.where(
                aranged_token_idx >= text_token_indices.unsqueeze(1)
            )
            _, target_text_idx = torch.where(
                aranged_token_idx < text_lengths.unsqueeze(1)
            )

            text_tokens_embeddings[batch_idx, target_text_idx] = token_embeds[
                batch_idx, text_token_idx
            ]
            text_tokens_mask[batch_idx, target_text_idx] = attention_mask[
                batch_idx, text_token_idx
            ]
        ...
        return (
            classes_embedding,
            classes_embedding_mask,
            text_tokens_embeddings,
            text_tokens_mask,
        )

    def get_loss(
        self, logits, labels, classes_embedding=None, classes_embedding_mask=None
    ):
        loss = None
        if labels is not None:
            ...
            if self.config.contrastive_loss_coef > 0 and classes_embedding is not None:
                contrastive_loss = sequence_contrastive_loss(
                    classes_embedding, classes_embedding_mask
                )
    ...
    def __init__(self, config: GLiClassModelConfig, from_pretrained=False):
        ...
        if config.encoder_config is None:
            if config.encoder_model_name is None:
                raise ValueError(
                    "You need to specify encoder model name to use it as a backbone."
                )
        ...
        if from_pretrained:
            self.encoder_model = ModelClass.from_pretrained(config.encoder_model_name)
        else:
            if decoder:
    ...
    def forward(
        ...
        **kwargs,
        ...
        (
            ...
            text_mask,
        ) = self._extract_class_features(encoder_layer, input_ids, attention_mask)
        ...
        if self.config.normalize_features:
            pooled_output = pooled_output / (
                pooled_output.norm(p=2, dim=-1, keepdim=True) + self.epsilon
            )

        classes_embedding = self.classes_projector(classes_embedding)
        if self.config.normalize_features:
            classes_embedding = classes_embedding / (
                classes_embedding.norm(p=2, dim=-1, keepdim=True) + self.epsilon
            )
...
class GLiClassEncoderDecoder(GLiClassBaseModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        pooled_output = self.dropout(pooled_output)
        if self.config.normalize_features:
            pooled_output = nn.functional.normalize(
                pooled_output, p=2, dim=-1, eps=self.epsilon
            )

        classes_embedding = self.classes_projector(classes_embedding)
        if self.config.normalize_features:
            classes_embedding = nn.functional.normalize(
                classes_embedding, p=2, dim=-1, eps=self.epsilon
            )
...
class GLiClassBiEncoder(GLiClassBaseModel):
    def __init__(self, config: GLiClassModelConfig, from_pretrained=False):
        ...
        if config.label_model_config is None:
            if config.label_model_name is None:
                raise ValueError(
                    "You need to specify label model name to use it as a backbone."
                )
            config.label_model_config = AutoConfig.from_pretrained(
                config.label_model_name
            )
        ...
        self.encoder_model = initialize_encoder(
            config.encoder_config, config.encoder_model_name, from_pretrained
        )
        self.label_encoder = initialize_encoder(
            config.label_model_config, config.label_model_name, from_pretrained
        )
    ...
    def pool_outputs(self, encoder_outputs):
        ...
        if self.config.normalize_features:
            text_embeddings = nn.functional.normalize(
                text_embeddings, p=2, dim=-1, eps=self.epsilon
            )
        return text_embeddings

    def encode_text(self, input_ids, attention_mask):
        outputs = self.encoder_model(
            input_ids.squeeze(1), attention_mask=attention_mask.squeeze(1)
        )
    ...
    def encode_classes(self, class_input_ids, class_attention_mask, labels_mask=None):
        ...
        if labels_mask is not None:
            ...
            outputs = self.label_encoder(
                selected_input_ids, attention_mask=selected_attention_mask
            )
            ...
            class_attention_mask = class_attention_mask.view(
                -1, class_input_ids.shape[-1]
            )
            outputs = self.label_encoder(
                class_input_ids, attention_mask=class_attention_mask
            )
        ...
        if self.config.normalize_features:
            class_embeddings = nn.functional.normalize(
                class_embeddings, p=2, dim=-1, eps=self.epsilon
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        class_embeddings = self.encode_classes(
            class_input_ids, class_attention_mask, labels_mask
        )
        logits = self.scorer(text_embeddings, class_embeddings) * self.logit_scale.to(
...
class GLiClassBiEncoderFused(GLiClassBiEncoder):
    ...
    def encode_text(self, input_ids, attention_mask, class_embeddings, labels_mask):
        ...
        selected_class_embeddings = class_embeddings[
            labels_batch_indices, labels_indices
        ]

        inputs_embeds[batch_indices, class_token_indices] = selected_class_embeddings
        encoder_outputs = self.encoder_model(
            inputs_embeds=inputs_embeds, attention_mask=attention_mask.squeeze(1)
        )

        post_class_embeddings = torch.zeros_like(class_embeddings)
        post_class_embeddings[labels_batch_indices, labels_indices] = encoder_outputs[
            0
    ...
    def forward(
        ...
        **kwargs,
        ...
        raw_class_embeddings = self.encode_classes(
            class_input_ids, class_attention_mask, labels_mask
        )
...
class GLiClassAudio(GLiClassBaseModel):
    def __init__(self, config: GLiClassModelConfig, from_pretrained=False):
        ...
        if config.audio_model_config is None:
            if config.audio_model_name is None:
                raise ValueError(
                    "You need to specify audio model name to use it as a backbone."
                )
            config.audio_model_config = AutoConfig.from_pretrained(
    ...
    def insert_audio_embeddings(self, inputs_embeds, input_ids, audio_embeddings):
        ...
        for batch_idx in range(input_ids.shape[0]):
            ...
            if len(audio_positions) > 0:
                inputs_embeds[batch_idx, audio_positions[0]] = audio_embeddings[
                    batch_idx
                ]
    ...
    def forward(self, input_ids, attention_mask, audio_input, labels=None, **kwargs):
        ...
        inputs_embeds = self.insert_audio_embeddings(
            inputs_embeds, input_ids, audio_embeddings
        )
        ...
        (
            ...
            audio_final_emb,
            audio_mask,
        ) = self._extract_class_features(outputs[0], input_ids, attention_mask)
...
class GLiClassModel(GLiClassPreTrainedModel):
    ...
    def get_input_embeddings(self):
        if self.config.architecture_type in {"uni-encoder", "audio-encoder"}:
            ...
            raise NotImplementedError(
                "Getting input embeddings is not implemented for bi-encoder architecture"
            )
    ...
    def set_input_embeddings(self, value):
        if self.config.architecture_type in {"uni-encoder", "audio-encoder"}:
            ...
            raise NotImplementedError(
                "Setting input embeddings is not implemented for bi-encoder architecture"
            )
    ...
    def tie_weights(self):
        if self.config.architecture_type in {"uni-encoder", "audio-encoder"}:
            ...
            raise NotImplementedError(
                "Tie weights is not implemented for bi-encoder architecture"
            )
    ...
    def resize_token_embeddings(
        ...
        if self.config.architecture_type in {"uni-encoder", "audio-encoder"}:
            model_embeds = self.model.encoder_model.resize_token_embeddings(
                new_num_tokens, pad_to_multiple_of
            )
        elif self.config.architecture_type == "encoder-decoder":
            model_embeds = self.model.encoder_decoder_model.resize_token_embeddings(
                new_num_tokens, pad_to_multiple_of
            )
            ...
            raise NotImplementedError(
                "Resizing is not implemented for bi-encoder architecture"
            )
