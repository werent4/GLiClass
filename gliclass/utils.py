...
def retrieval_augmented_text(text: str, examples: list) -> str:
    ...
    relevant_examples = [
        ex for ex in examples if set(ex.get("true_labels", [])) & all_labels
    ]
    ...
    for example in relevant_examples:
        ...
        false_labels_str = " ".join(
            [f"<<FALSE_LABEL>> {label}" for label in false_labels]
        )
    ...
    augmented_text = (
        f"{text} {' '.join(retrieved_examples)}" if retrieved_examples else text
    )
