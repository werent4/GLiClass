...
from transformers import (
    ZeroShotClassificationPipeline as TransformersClassificationPipeline,
)
            ...
            with torch.no_grad():
                ...
                if not hasattr(outputs, "loss"):
                    raise AttributeError(
                        "Model output does not contain 'loss' attribute"
                    )
                loss = outputs.loss

                if not hasattr(outputs, "logits"):
                    raise AttributeError(
                        "Model output does not contain 'logits' attribute"
                    )
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            ...
                            if (
                                ...
                                and n not in encoder_parameters
                                and p.requires_grad
                            )
                        ],
...
class RLTrainerConfig(TrainingArguments):
    ...
    labels_smoothing: float = field(
        default=-1, metadata={"help": "Labels smoothing factor."}
    )
    entropy_beta: float = field(
        default=-1, metadata={"help": "Coeficient of entropy factor."}
    )
    kl_beta: float = field(
        default=-1, metadata={"help": "Coeficient of KL-divergence factor."}
    )
    get_actions: str = field(
        default="bernoulli",
        metadata={
            "help": "How to get actions of a model, default is `bernoulli`, another option is `threshold`"
        },
    )
...
class RLTrainer(Trainer):
    def __init__(
        ...
        reference_model: Optional[
            Union[ZeroShotClassificationPipeline | TransformersClassificationPipeline]
        ] = None,
        ...
        with torch.no_grad():
            if isinstance(self.reference_model, ZeroShotClassificationPipeline):
                ...
                raise NotImplementedError(
                    "This classification pipelines is not supported as a reference model."
                )
        max_length = max(len(seq) for seq in all_scores)
        all_scores = torch.FloatTensor(
            [seq + [0] * (max_length - len(seq)) for seq in all_scores]
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if value_outputs is not None:
            state_values = value_outputs.logits[:, 0].unsqueeze(
                -1
            )  # Using first token logits as value prediction
        ...
        if self.args.label_smoothing_factor > 0:
            smoothed_actions = (
                actions * (1 - self.args.label_smoothing_factor)
                + 0.5 * self.args.label_smoothing_factor
            )
            log_prob_current = smoothed_actions * torch.log(probs + 1e-8) + (
                1 - smoothed_actions
            ) * torch.log(1 - probs + 1e-8)
        else:
            log_prob_current = actions * torch.log(probs + 1e-8) + (
                1 - actions
            ) * torch.log(1 - probs + 1e-8)
        ...
        if self.args.alpha >= 0:
            alpha_t = self.args.alpha * original_targets + (1 - self.args.alpha) * (
                1 - original_targets
            )
    ...
    def _inner_training_loop(self, *args, **kwargs):
        ...
        if self.value_model is not None:
            value_optimizer = torch.optim.Adam(
                self.value_model.parameters(), lr=self.args.learning_rate
            )
        args = self.args
                ...
                for _ in range(args.num_rl_iters):
                    try:
                        ...
                        if self.reference_model is not None:
                            reference_probs = self.get_reference_scores(
                                input_texts, labels_text
                            )
                        else:
                    ...
                    if self.args.max_grad_norm is not None:
                        ...
                        if self.value_model is not None:
                            torch.nn.utils.clip_grad_norm_(
                                self.value_model.parameters(), self.args.max_grad_norm
                            )
