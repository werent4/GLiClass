...
class GLiClassDataset(Dataset):
    ...
    def prepare_labels(self, example, label2idx, problem_type):
        if problem_type == "single_label_classification":
            ...
            if isinstance(example["true_labels"], dict):
                labels = [
                    example["true_labels"][label]
                    if label in example["true_labels"]
                    else 0.0
                    for label in example["all_labels"]
                ]
            else:
                labels = [
                    1.0 if label in example["true_labels"] else 0.0
                    for label in example["all_labels"]
                ]
        else:
    ...
    def tokenize(self, texts):
        tokenized_inputs = self.tokenizer(
            texts, truncation=True, max_length=self.max_length, padding="longest"
        )
        return tokenized_inputs

    def tokenize_labels(self, labels):
        tokenized_inputs = self.labels_tokenizer(
            labels, truncation=True, max_length=self.max_length, padding="longest"
        )
        return tokenized_inputs
    ...
    def tokenize_and_prepare_labels_for_biencoder(self, example):
        ...
        tokenized_inputs["class_input_ids"] = torch.tensor(
            tokenized_classes["input_ids"]
        )
        tokenized_inputs["class_attention_mask"] = torch.tensor(
            tokenized_classes["attention_mask"]
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.get_negatives and random.randint(0, 1):
            ...
            new_negatives = random.sample(
                self.dataset_labels, k=random.randint(1, max_negatives)
            )
