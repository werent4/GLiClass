...
class TestModel:
    ...
    def load_model(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_name, token=self.token, add_prefix_space=True
        )
    ...
    def process(self):
        ...
        for dataset in self.datasets:
            ...
            if dataset == "SetFit/sst5":
                ...
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, classes=classes
                )
            elif dataset == "PolyAI/banking77":
                ds = load_dataset(dataset, trust_remote_code=True)
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, text_column="text", label_column="label"
                )
                ...
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, text_column="sentence", label_column="label"
                )
            elif dataset == "AmazonScience/massive":
                ds = load_dataset(dataset, "en-US")
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, text_column="utt", label_column="intent"
                )
            elif dataset == "stanfordnlp/imdb":
                ds = load_dataset(dataset, trust_remote_code=True)
                classes = ["negative", "positive"]
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, classes=classes, text_column="text", label_column="label"
                )
                ...
                ds = load_dataset(dataset, trust_remote_code=True)
                test_texts, classes, true_labels = self.prepare_nomapping(
                    ds, text_column="text", label_column="labels"
                )
